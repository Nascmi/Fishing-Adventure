...
Boats and reusable lures form the foundation of Phase 4. Boats are permanent purchases that open authored viewpoints without driving, fuel, or maintenance. Every location retains included general-purpose tackle. A curated affordable catalog provides 5% relative affinity to recognizable fish groups, while expensive permanent specialty lures provide 20% relative affinity for that location's legendary fish without guaranteeing it. Rod quality also reduces rare quiet casts from 3% to 1.5%, with streak protection preventing consecutive quiet casts. New locations, aquariums, photography, legendary quests, and other extensions are not approved merely because they appear here; each must pass the feature approval test and enter the roadmap before implementation.
