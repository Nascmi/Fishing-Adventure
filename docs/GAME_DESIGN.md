...
Boats and reusable lures form the foundation of Phase 4. Boats are permanent, location-aware purchases that open authored viewpoints without driving, fuel, or maintenance. Great Lake uses a modest aluminum skiff for Weed Edge and Rocky Drop-off; Gulf Coast uses a shallow-draft Bay Skiff for Oyster Reef and Tidal Channel. Each location retains an included shore or bank area so buying a boat expands choice rather than replacing earlier fishing. Every location also retains included general-purpose tackle. A curated affordable catalog provides 5% relative affinity to recognizable fish groups, while expensive permanent specialty lures provide 20% relative affinity for that location's legendary fish without guaranteeing it. Rod quality reduces rare quiet casts from 3% to 1.5%, with streak protection preventing consecutive quiet casts. New locations, aquariums, photography, legendary quests, and other extensions are not approved merely because they appear here; each must pass the feature approval test and enter the roadmap before implementation.

Boat ownership is not universal progression. Pine River remains a wading-and-fly-fishing experience, Open Gulf already places the player aboard a charter vessel, and the current Backyard Pond is too intimate to support believable boat-access areas. Open Gulf offers captain-selected Blue Water, Working Rig, and Reef Edge positions aboard the existing charter. Each actual relocation costs 250 coins for fuel and captain time; arriving at Blue Water and remaining at the current position are free. Boats and their cosmetic surfaces exist only where the setting calls for them, never because the project needs another saleable slot.
